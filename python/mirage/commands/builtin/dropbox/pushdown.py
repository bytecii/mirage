...
async def narrow_scope(
    ...
    *,
    ...
    ``content_search``, the scan is recursive, a whole-word literal can be
    pushed down (``whole_word_literal``, which is what makes a word-based
    search complete), and the output mode tolerates a narrowed-superset
    file set (``exact_file_set`` covers flags like -v that must see every
    file). Unlike the GitHub narrow there is no scope-size gate: one search
    call plus targeted downloads beats a readdir-walk-plus-download-
    everything scan at every scope size, and Dropbox search has no
    code-search-style rate ceiling. An empty search result still falls back
    to the full scan (GitHub parity) because search indexing lags recent
    writes.
    ...
    if (query is not None and recursive and not exact_file_set
