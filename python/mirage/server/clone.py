...
async def build_override_mounts(
    ...
    Returns:
        dict[str, Any]: prefix -> built VFS, empty when the
...
async def clone_workspace_with_override(src_ws: Workspace,
    ...
    Behavior:
        * Local mounts (RAM, Disk) are reconstructed fresh, so the
          clone's writes never touch the original's data.
        * Remote mounts (S3, Redis, GDrive, ...) that redact secrets
          or connection material are reused from the original by default
          -- they share connection pools and bucket data.
        * If ``override`` supplies a fresh VFS for a prefix, that
          VFS replaces the reused one -- e.g. point the clone at
