...
def test_quote_ident_escapes_embedded_quotes():
    assert client.quote_ident("users") == '"users"'
    assert client.quote_ident('a"b') == '"a""b"'


def test_qualified_composes_both_parts():
    assert client.qualified("public", "users") == '"public"."users"'


def test_quote_ident_neutralizes_injection():
    payload = 'books" CROSS JOIN secret AS "s'
    quoted = client.quote_ident(payload)
...
def test_canonicalize_value_drops_trailing_zero_on_whole_floats():
    ...
    assert client.canonicalize_value(5.0) == 5
    assert isinstance(client.canonicalize_value(5.0), int)
    assert client.canonicalize_value(4.5) == 4.5
    assert isinstance(client.canonicalize_value(4.5), float)


def test_canonicalize_value_leaves_non_floats_and_specials():
    assert client.canonicalize_value(True) is True
    assert client.canonicalize_value("5.0") == "5.0"
    assert client.canonicalize_value(float("inf")) == float("inf")
...
async def test_list_matviews(mock_conn):
    ...
    result = await client.list_matviews(mock_conn, "public")
    assert result == ["mv1"]
...
async def test_count_rows(mock_conn):
    ...
    result = await client.count_rows(mock_conn, "public", "users")
    assert result == 42
...
async def test_estimated_row_count(mock_conn):
    ...
    result = await client.estimated_row_count(mock_conn, "public", "users")
    assert result == 9999
...
async def test_table_size_bytes(mock_conn):
    ...
    result = await client.table_size_bytes(mock_conn, "public", "users")
...
async def test_fetch_primary_key(mock_conn):
    ...
    result = await client.fetch_primary_key(mock_conn, "public", "users")
    assert result == ["id"]
