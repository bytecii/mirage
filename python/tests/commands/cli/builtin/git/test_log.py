...
async def test_oneline_lists_newest_first(git_ws):
    result = await git_ws.shell("git -C /repo log --oneline")
...
async def test_oneline_rows_start_with_a_seven_character_id(git_ws):
    result = await git_ws.shell("git -C /repo log --oneline -n 1")
...
async def test_entries_are_separated_by_a_blank_line(git_ws):
    result = await git_ws.shell("git -C /repo log")
...
async def test_a_revision_operand_starts_the_walk_there(git_ws):
    result = await git_ws.shell("git -C /repo log HEAD~1 --oneline")
...
async def test_pickaxe_ignores_a_rewrite_that_keeps_the_count(git_ws):
    result = await git_ws.shell("git -C /repo log -S one --oneline")
    assert subjects_of(result.stdout) == ["first"]


@pytest.mark.asyncio
async def test_an_unknown_revision_is_a_fatal(git_ws):
    result = await git_ws.shell("git -C /repo log nope --oneline")
...
async def test_a_date_git_reads_but_we_do_not_is_a_fatal(git_ws):
    result = await git_ws.shell("git -C /repo log --since '2 weeks ago'")
    assert result.exit_code == 128
...
async def test_a_window_that_excludes_everything_prints_nothing(git_ws):
    result = await git_ws.shell("git -C /repo log --since 4102444800")
    assert result.exit_code == 0
...
async def test_all_includes_commits_from_other_branches(git_ws, repo_path):
    ...
    plain = await git_ws.shell("git -C /repo log --oneline")
    everything = await git_ws.shell("git -C /repo log --all --oneline")
...
async def test_format_separator_and_terminator_semantics(git_ws):
    ...
    separated = await git_ws.shell("git -C /repo log --pretty='format:%s'")
    terminated = await git_ws.shell("git -C /repo log --format='%s'")
...
async def test_pretty_oneline_prints_full_ids_unlike_oneline(git_ws):
    pretty = await git_ws.shell("git -C /repo log --pretty=oneline -n 1")
    dashed = await git_ws.shell("git -C /repo log --oneline -n 1")
...
async def test_bare_pretty_defaults_to_medium(git_ws):
    bare = await git_ws.shell("git -C /repo log --pretty")
    medium = await git_ws.shell("git -C /repo log")
    assert bare.stdout == medium.stdout


@pytest.mark.asyncio
async def test_block_presets_shape_their_headers(git_ws):
    short = await git_ws.shell("git -C /repo log -n 1 --pretty=short")
    full = await git_ws.shell("git -C /repo log -n 1 --pretty=full")
    fuller = await git_ws.shell("git -C /repo log -n 1 --pretty=fuller")
...
async def test_empty_format_prints_nothing(git_ws):
    result = await git_ws.shell("git -C /repo log --format=")
    assert result.exit_code == 0
...
async def test_unknown_placeholders_stay_verbatim(git_ws):
    result = await git_ws.shell("git -C /repo log -n 1 --format='%q %zz'")
...
async def test_format_empty_entries_keep_their_separators(git_ws):
    ...
    result = await git_ws.shell("git -C /repo log --pretty=format:")
    assert result.stdout == b"\n\n"
...
async def test_tformat_terminates_empty_entries(git_ws):
    ...
    result = await git_ws.shell("git -C /repo log --format='%d'")
    assert result.stdout == b" (HEAD -> main)\n\n\n"


@pytest.mark.asyncio
async def test_x_placeholder_names_a_raw_byte(git_ws):
    result = await git_ws.shell("git -C /repo log -n 1 --format='a%x80b'")
