...
async function checkNoClobber(ws: Workspace, dst: string, label: string): Promise<void> {
...
async function checkOmitDirectory(ws: Workspace, dst: string, label: string): Promise<void> {
  const [, err, code] = await run(ws, `cp /ram/dir ${dst}/nope`)
  check(`${label}: cp dir without -r fails`, code === 1 && err.includes('omitting directory'))
}
...
async function checkReadFamily(ws: Workspace, dst: string, label: string): Promise<void> {
  ...
  check(`${label}: tail banners`, out.includes(`==> ${src} <==`) && out.includes('bbb'))
  ;[out] = await run(ws, `wc -l ${src} ${copied}`)
  check(`${label}: wc total`, out.includes('total'))
  ;[out] = await run(ws, `grep aaa ${src} ${copied}`)
  check(`${label}: grep prefixes`, out.includes(`${src}:aaa`) && out.includes(`${copied}:aaa`))
  ;[out] = await run(ws, `rg aaa ${src} ${copied}`)
  check(`${label}: rg prefixes`, out.includes(`${src}:aaa`) && out.includes(`${copied}:aaa`))
...
async function checkPartialRead(ws: Workspace, dst: string, label: string): Promise<void> {
  const src = '/ram/dir/a.txt'
  const miss = `${dst}/copied/nope.txt`
  let [out, err, code] = await run(ws, `cat ${src} ${miss}`)
  check(
    `${label}: cat keeps partial output`,
    out === 'aaa\n' && code === 1 && err === `cat: ${miss}: No such file or directory\n`,
  )
  ...
  check(
    ...
    out === '     1\taaa\n' && code === 1 && err === `nl: ${miss}: No such file or directory\n`,
  )
...
async function checkCompare(ws: Workspace, dst: string, label: string): Promise<void> {
  ...
  check(`${label}: diff differing`, code === 1 && out.includes('aaa') && out.includes('bbb'))
...
async function checkCdCrossMount(ws: Workspace, dst: string, label: string): Promise<void> {
  ...
  check(`${label}: CDPATH spans mounts`, lines[lines.length - 1] === `${dst}/copied`)
}

// mv a directory across mounts: destination gets the tree, source is gone.
async function checkMove(ws: Workspace, dst: string, label: string): Promise<void> {
...
async function checkSymlinks(ws: Workspace, dst: string, label: string): Promise<void> {
  ...
  check(`${label}: ln onto existing link fails`, code === 1 && err.includes('File exists'))
  ;[, , code] = await run(ws, `ln -sf ${dst}/copied/sub/b.txt /ram/sl.txt`)
  ;[out] = await run(ws, 'readlink /ram/sl.txt')
  check(`${label}: ln -sf re-points link`, code === 0 && out.trim() === `${dst}/copied/sub/b.txt`)
  ;[out, , code] = await run(ws, `readlink ${dst}/copied/a.txt`)
  check(`${label}: readlink non-link exits 1`, code === 1 && out === '')
  ;[out] = await run(ws, 'ls -l /ram')
  check(`${label}: ls -l shows link arrow`, out.includes(`sl.txt -> ${dst}/copied/sub/b.txt`))
  ...
  check(`${label}: mv link across mounts keeps target`, out.trim() === '/ram/sl.txt')
  ...
  ;[out] = await run(ws, 'ls -l /ram')
  check(`${label}: dangling link still listed`, out.includes(`dl.txt -> ${dst}/copied/nope.txt`))
...
async function checkMetadata(
  ...
  check(`${label}: chown lands on dst mount`, st.uid === 500 && st.gid === 'dev')
  check(`${label}: touch stamps dst mtime`, (st.modified ?? '').startsWith('2026-01-02T15:30'))
  ...
  check(`${label}: chmod through cross-mount link hits target`, st.mode === 0o640)
  check(
  ...
  check(`${label}: touch creates through cross-mount dir link`, out.includes('created.txt'))
  await run(ws, `rm /ram/ml.txt /ram/mdir ${dst}/copied/m.txt ${dst}/copied/created.txt`)

  await run(ws, `printf 'nnn\n' > ${dst}/copied/n.txt`)
  await run(ws, `chmod g+w ${dst}/copied/n.txt`)
  st = await statOf(ws, `${dst}/copied/n.txt`)
  check(`${label}: symbolic chmod applies against 644 base`, st.mode === 0o664)
  ...
  check(`${label}: touch -c does not create`, code === 0 && !lsAll.includes('absent.txt'))
  await run(ws, `ln -s ${dst}/copied/n.txt /ram/nl.txt`)
  await run(ws, 'chown -h 501:ops /ram/nl.txt')
  st = await statOf(ws, `${dst}/copied/n.txt`)
  check(`${label}: chown -h leaves target owner`, st.uid === 500 && st.gid === 'dev')
  ...
  await run(ws, `printf 'nnn\n' > ${dst}/copied/n.txt`)
  st = await statOf(ws, `${dst}/copied/n.txt`)
  check(`${label}: rm drops meta for recreated file`, st.mode === null || st.mode === undefined)
  await run(ws, `rm /ram/nl.txt /ram/tref.txt ${dst}/copied/n.txt`)
}

async function checkSymlinkCache(ws: Workspace, client: S3Client, label: string): Promise<void> {
...
async function checkCrossMountCache(ws: Workspace, client: S3Client, label: string): Promise<void> {
  ...
  check(`${label}: cross tail serves cached`, out.includes('last') && !out.includes('nomatch'))
...
async function checkGlobCache(ws: Workspace, client: S3Client, label: string): Promise<void> {
  ...
  check(`${label}: glob head serves cached`, out.includes('alpha-v1') && !out.includes('alpha-v2'))
  ;[out] = await run(ws, `tail -n 1 ${g}`)
  check(`${label}: glob tail serves cached`, out.includes('alpha-v1') && !out.includes('EXTRA'))
  ;[out] = await run(ws, `wc -l ${g}`)
  check(`${label}: glob wc serves cached`, out.includes('2 total') && !out.includes('3 total'))
  ;[out] = await run(ws, `grep alpha-v1 ${g}`)
  check(`${label}: glob grep serves cached`, out.includes('/s3/glob/a.txt:alpha-v1'))
}

async function checkWhoami(ws: Workspace): Promise<void> {
  const [first, , code] = await run(ws, 'whoami')
  let out = first
  check('whoami: prints the launch agentId', out === 'integ-agent\n' && code === 0)
