...
CASES: list[tuple[str, str]] = [
    ...
    ("source_prep", "mkdir -p /data/g10 && cd /data/g10"
     " && printf 'echo sourced-ok\\n' | tee lib.sh > /dev/null"),
    ("source_relative",
     "source lib.sh && . /data/g10/lib.sh && cd / && rm -r /data/g10"),
    ("relative_operand_prep", "mkdir -p /data/g11/sub && cd /data/g11"
     " && printf 'one\\ntwo\\n' | tee sub/README > /dev/null"),
    ("relative_operand_extensionless",
     "cat sub/README && wc -l sub/README && head -1 sub/README"),
    ...
    ("relspell_labels", "wc -l sub/a.txt && head -v sub/a.txt"
     " && cd / && rm -r /data/g14"),
    ("midglob_prep", "mkdir -p /data/g15/sub /data/g15/sea && cd /data/g15"
    ...
    ("arch_tar_create_verbose",
     "tar -c -v -z -f /data/arch/a.tgz /data/arch/g.txt"),
    ...
    ("arch_tar_exclude", "echo noise | tee /data/arch/skip.log > /dev/null"
     " && tar -c -v -f /data/arch/b.tar --exclude '*.log'"
     " /data/arch/g.txt /data/arch/skip.log"),
    ("arch_zip_unzip",
     "zip -q /data/arch/z.zip /data/arch/g.txt && unzip -p /data/arch/z.zip"),
    ("arch_split_roundtrip", "split -b 4 /data/arch/g.txt /data/arch/pt_"
     " && cat /data/arch/pt_aa /data/arch/pt_ab"),
    ("arch_csplit", "cat /data/a.txt | tee /data/arch/c.txt > /dev/null"
...
EXIT_CODE_CASES: list[tuple[str, str]] = [
    ...
    ("pipe_concat_head_first",
     "cat /data/sorted_a.txt /data/sorted_b.txt | head -n 2"),
