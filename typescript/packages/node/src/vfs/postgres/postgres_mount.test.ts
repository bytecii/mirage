...
import { PostgresVFS } from './postgres.ts'
...
describe('PostgresVFS mount integration', () => {
  let ws: Workspace
  let vfs: PostgresVFS
  ...
  beforeEach(() => {
    ...
    largeRows = 5
    largeWidth = 64
    vfs = new PostgresVFS({ dsn: 'postgres://localhost/acme' })
    ws = new Workspace({ '/pg': vfs }, { mode: MountMode.READ })
  })
