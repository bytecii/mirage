...
| `workspaceId` | none | Restrict to a single workspace. The `trello` commands refuse an id outside it. |
| `boardIds` | none | Optional allowlist of board IDs. The `trello` commands refuse an id outside it. |
