...
`MongoDBVFS` exposes a MongoDB connection as a read-only filesystem: databases at the top level, collections inside, individual documents at the leaves.
...
```ts
import { MongoDBVFS, MountMode, Workspace } from '@struktoai/mirage-browser'
...
| `maxDocLimit` | `5000` | Most documents one `tail -n` returns; past it, a notice on stderr and exit 1. |

## Mount mode

`read`.

For the database/collection/document layout see the [Python MongoDB docs](/python/vfs/mongodb).
