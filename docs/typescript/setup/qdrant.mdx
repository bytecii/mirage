...
mirage depends on no model runtime; `examples/typescript/qdrant/qdrant.ts`
uses `@huggingface/transformers` with `Xenova/all-MiniLM-L6-v2`, the ONNX
export of the default model. Without it the query text goes to the server, so
...
resource. Browsing (`ls`/`cat`/`find`/`grep`) needs no embedding.
