...
a field to preserve its complete value in one path-safe segment: `/` renders as
`∕`, and a value that already holds `∕` or `⁄` has that character prefixed with
`⁄`, so no two values share a segment. Basenames must be unique within a parent
