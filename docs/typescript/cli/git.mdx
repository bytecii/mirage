...
import { RAMVFS, Workspace } from '@struktoai/mirage-node'

const ws = new Workspace({ '/repo': new RAMVFS() })
...
| `commit`   | `-m` required, `--author "Name <email>"` optional, `-a` restages tracked files first                                                     |
