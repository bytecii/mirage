...
```typescript
import { EmailVFS, HIMALAYA, Workspace } from '@struktoai/mirage-node'

const ws = new Workspace({ '/mail': new EmailVFS(config) })
...
| [airtable](/typescript/cli/airtable)     | Airtable         | noun/verb (`record update`, `comment add`)  |
