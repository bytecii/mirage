---
title: Watch Matrix
description: TypeScript watch delivery support and the pull or push detection paths available for each VFS.
icon: table-cells
---
...
| Box                   | ✓ `/events` stream (walk is reset) | content `sha1` | Box `/events` long poll, `BoxEventHook` |
...
async function* walk(root: PathSpec): AsyncGenerator<WalkEntry> {
  for await (const object of listObjects(root.vfsPath)) {
    yield {
...
backends with a native cursor can do the same. The opaque checkpoint can be a
Dropbox cursor, a Microsoft Graph delta link, or any other serialized provider
state. Keep the walk as the reset path, though. A cursor is a promise the
