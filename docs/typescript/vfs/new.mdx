...
- Declare native text search in `new VFSAdapter({ read, search })`. Generic grep/rg builders consume it automatically. The hierarchy kit supplies `makeSearchOp` for scope-based core functions; unsupported requests fall back to scanning. Existing wrappers may call `runSearch(X_IO, name, ...)` when preserving a custom provision or registration.
