...
A **deny** refuses the line at 126. A command entry is a token-prefix pattern, so `rm` would cover every `rm` line while `rm -rf` covers only the recursive ones:

```
$ rm -rf /data/build
rm: policy denied: no recursive deletes
```
