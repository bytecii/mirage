...
and auto-loaded scientific packages. `open()` and `os.listdir` reach mounted paths through a worker bridge. Only accessed files are fetched, and each run sees fresh workspace state. Browsers without `SharedArrayBuffer` use the eager fallback described under [Python filesystem](/typescript/python-fs).
