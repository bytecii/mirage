...
<Tabs>
  <Tab title="Read a file">
    ...
    ```
    The worker fetches that file on first access. Unrelated files and mounts are not read; the per-run cache is discarded before the next command.
  </Tab>
  <Tab title="Write a file">
    ...
    ```
    `close()` records the write; before a later backend read or when the script finishes, the bytes go back through the resource's `write` op, or its `append` op when the handle only extended the file.
  </Tab>
  <Tab title="List a directory">
    ...
    for msg_dir in os.listdir('/gmail/INBOX/2026-05-03/'):
        print(msg_dir)
    ```
    A synthesized path is materialized when Python lists or opens it.
  </Tab>
  ...
  <Tab title="Symlinks & metadata">
    ...
    the same way it lands on a RAM one. Links the shell made are resolved on access, as links rather than as copies of their targets, so a
...
A mount at `/` cannot replace Pyodide's own filesystem, which holds its standard library. If a non-root shell cwd belongs to that unsupported mount, Python runs from the interpreter's existing cwd so filesystem-independent code still works. `/` itself remains usable as a cwd. Mount workspace files under a non-root prefix to access them from Python. Supported child mounts keep their normal cwd and relative file access even when a root mount is also present.
...
| GitHub, Redis | ✅ |
| Gmail (including synthesized date dirs) | ✅ (materialized on access) |
| Slack | ✅ (only accessed paths are fetched) |

## Runtime requirements

Lazy filesystem access runs Pyodide in a worker and requires `SharedArrayBuffer`. It works in Node without experimental flags. Browser pages must be cross-origin isolated, typically by serving `Cross-Origin-Opener-Policy: same-origin` and `Cross-Origin-Embedder-Policy: require-corp`; their bundler must include the module worker.

Vite consumers must select ES module output for workers, including when importing the browser workspace package:
...
Without workers or `SharedArrayBuffer`, or when a worker cannot start (for example under CSP), Mirage retains the eager fallback: it collects each mount before execution and replays writes afterward. This fallback preserves filesystem support but still enumerates and reads whole mounts. Neither path requires JSPI.
...
|---|---|
| `FileNotFoundError [Errno 44]` | The workspace could not resolve the requested path. |
