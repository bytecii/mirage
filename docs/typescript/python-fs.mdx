...
Without workers or `SharedArrayBuffer`, Mirage retains the eager fallback: it collects each mount before execution and replays writes afterward. This fallback preserves filesystem support but still enumerates and reads whole mounts. Neither path requires JSPI.
