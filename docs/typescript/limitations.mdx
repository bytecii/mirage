...
Browsers can't mount filesystems. `Workspace({ fuse: true })` throws on construction. Use `ws.execute(...)` (virtual executor) and `ws.fs.readFile/writeFile` instead. Every builtin (`cat`, `grep`, `jq`, `awk`, `python3`, etc.) is reimplemented in-process, so most agent code paths work unchanged from Node.
...
| `Workspace({ fuse: true })` | Browser | ❌ throws; not supported |
