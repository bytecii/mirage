...
<Tabs>
  <Tab title="Use FUSE instead">
    ...
    const ws = new Workspace({ '/data': new Mount(new RAMResource(), { backend: MountBackend.FUSE }) })
    await ws.fuseReady()
...
Browsers can't mount filesystems. The browser `Workspace` takes plain resources: there is no `Mount` backend option and no `addFuseMount`. Use `ws.execute(...)` (virtual executor) and `ws.fs.readFile/writeFile` instead. Every builtin (`cat`, `grep`, `jq`, `awk`, `python3`, etc.) is reimplemented in-process, so most agent code paths work unchanged from Node.
...
Pyodide's worker fetches workspace files on access and caches them for that
execution. An external edit to a cached file is not visible until the next
run. Without workers or `SharedArrayBuffer`, the fallback collects whole
mounts before execution, which can be expensive for large resources.

See [Pyodide workspace file access](/typescript/runtime/python#workspace-file-access)
for browser requirements and limitations.

### 5. Some resource drivers remain Node-only

SSH, LanceDB, Email, Disk, and the Hugging Face resources are only exposed by `@struktoai/mirage-node` because their drivers need Node APIs or raw TCP. Importing them from `@struktoai/mirage-browser` is a build error.
...
| `Workspace` with a fuse `Mount` | Browser | ❌ not supported; the browser `Workspace` takes plain resources |
...
| Node-only resources (SSH, LanceDB, Email, Disk, Hugging Face, GridFS, FUSE) | Browser | ❌ Node APIs or raw TCP required |
