...
<Columns cols={2}>
  <Card title="Captured" icon="check">
    ...
    - Profiles as documents, and which one is the default
    - The workspace consistency policy
    - Sessions in full: cwd, variables, per-mount modes, hides and shows, hidden variables, admission rules, pending and answered asks
    - History, finished jobs, namespace nodes (symlinks, attribute overlay)
    ...
    - The class names of coded policies (named, not carried)
  </Card>
  <Card title="Not captured" icon="xmark">
    ...
    - Coded policies, secret stores, runtimes: code the loader registers
    - Shell functions, aliases, `shopt` and `set -o` state
  </Card>
</Columns>
...
- **A restored session is never wider** than the target's document or than the live session it lands on. On a running workspace a `checkout` can add a version's restrictions to a live session, never lift the live ones; `set_session_profile` is the host's reset. A loader that supplies a *stricter* version of a profile a table names governs the restored session under it.
- **A show survives only where the other side leaves its anchor open.** A show exists to re-open what a hide covers, so the question is asked of the other side's hides, not of the merged set: a table that never mentioned a hidden subtree does not take that subtree's carve-out away with it. A show only one side states is dropped where the other side hides its anchor, mode and all.
