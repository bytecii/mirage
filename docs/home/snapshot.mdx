...
Four rules follow, in both languages:
...
- **A restored session is never wider** than the target's document or than the live session it lands on. On a running workspace a `checkout` can add a version's restrictions to a live session, never lift the live ones; `set_session_profile` is the host's reset.
