...
`mirage:idx:directory:v3:`. Invalidation uses permanent keys independent of the
...
Metadata uses `mirage:idx:entry:v3:` so listings and their metadata both
start cold after an upgrade. Existing list-format cache records and entry rows
are not reused, and neither are `v2` rows: the entry payload became the one
snake_case document both languages write, where the TypeScript releases before
`v3` wrote camelCase, so a `v3` worker never opens a `v2` row rather than
decoding two formats. The first lookup fetches current backend metadata.
...
The unversioned cache format is unsupported. Upgrade all workers that share
an index together; mixing workers that do not implement the shared-token
protocol is unsupported. Current workers do not read or clean up the old
payload keys.
