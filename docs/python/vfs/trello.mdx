...
icon: table-columns
---
The Trello VFS exposes workspaces, boards, lists, cards, members, and
labels as a virtual filesystem mounted at some prefix such as `/trello/`.

For API key setup, see [Trello Setup](/home/setup/trello).
...
ws = Workspace({"/trello": vfs}, mode=MountMode.READ)
```
...
VFS-specific commands can reference the correct VFS without
extra lookups.
...
Uses `IndexCacheStore` (same as other mounts).
...
`workspace_id` and `board_ids` narrow these commands the way they narrow the
tree: `trello board list` shows the boards the mount lists, and a board, list
or card id on a board outside them is refused before any read or write
(`trello card show: card crd_009 is outside this mount's scope`, exit 1).
