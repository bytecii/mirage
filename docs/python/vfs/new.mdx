...
Create an `Accessor` subclass that owns the client; the base class gives it `pool`, the `SessionPool` every request of the mount rides. Add the VFS name to `VFSName`, export the config and VFS from `vfs/<name>/__init__.py`, and add a lazy `VFSEntry` to `mirage/vfs/registry.py`. The registry entry is required for YAML, snapshots, and the daemon to construct the VFS.
