...
---
The GridFS VFS mounts a MongoDB GridFS bucket at some prefix such as
...
from mirage.vfs.gridfs import GridFSVFS, GridFSConfig
...
vfs = GridFSVFS(config)
ws = Workspace({"/gridfs": vfs}, mode=MountMode.WRITE)
```
...
keeps the old ones. The VFS always reads and lists the newest revision
...
an ordinary MongoDB collection, so a `find` that can only print files
(`-type f`) pushes its filters into the `fs.files` query itself:

- `-name` / `-iname` globs become anchored regexes on the filename
- `-type f` drops the directory markers
- `-size` bounds map to the stored `length`

`find /gridfs/ -type f -name '*.csv' -size +1M` narrows inside MongoDB and
only matching metadata comes back, so `find` stays cheap even on large
buckets. Without `-type f` the filters run on the results instead: a
directory that exists only because files sit under it has no document of
its own, so a narrowed query would lose it. Semantics are identical to
every other backend: the shared predicate evaluation still runs on the
results.

## Cache

The GridFS VFS uses `IndexCacheStore` with `index_ttl = 600`
...
async def main() -> None:
    ...
    await ws.shell("echo hello | tee /gridfs/notes/a.txt > /dev/null")

    r = await ws.shell("ls /gridfs/")
    print(await r.stdout_str())

    r = await ws.shell("cat /gridfs/notes/a.txt")
    print(await r.stdout_str())

    r = await ws.shell("find /gridfs/ -name '*.txt'")
    print(await r.stdout_str())

    r = await ws.shell("grep hello /gridfs/notes/a.txt")
    print(await r.stdout_str())

    r = await ws.shell("stat /gridfs/notes/a.txt")
    print(await r.stdout_str())
...
The GridFS VFS supports the same full command surface as the
[S3 VFS](/python/vfs/s3): read commands (`cat`, `head`, `tail`,
