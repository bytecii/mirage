---
title: Dropbox
description: Mount a Dropbox account (or a single folder of it) as a read-only Mirage filesystem with async access and shell commands.
icon: dropbox
---
...
`/2/files/list_folder` for directories and `/2/files/download` for content.
Files are served as raw bytes. The mount is read-only.
...
ws = Workspace({"/dropbox": resource}, mode=MountMode.READ)
```
