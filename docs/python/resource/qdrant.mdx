...
value has its own segment that lists and opens. Basenames must be unique within
their parent group; Mirage refuses an ambiguous listing instead of silently
merging two source documents, and opening a basename directory checks every
point of its parent group, so a second source past `max_rows` is refused rather
than hidden.
