...
cache. A label that is not a string spells the way the point's `.json` spells
it (`true`, `1`), the same in Python and TypeScript. `basename_fields` strips
URL/path parents from the named `group_by` fields. Leave a field out to
preserve its complete value in one path-safe segment: `/` renders as `∕`, a
value that already holds `∕` or `⁄` has that character prefixed with `⁄`, and a
blank value or one that starts with `.` is prefixed with `⁄` as well, so every
value has its own segment that lists and opens. A basename longer than 255
bytes, which ext4 and APFS refuse, is cut to fit and ends in `__` plus the md5
of the whole name, so two long leaves stay two directories. Basenames must be
unique within their parent group; Mirage refuses an ambiguous listing instead
of silently merging two source documents, and opening a basename directory
checks every point of its parent group, so a second source past `max_rows` is
