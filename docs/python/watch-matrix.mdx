---
title: Watch Matrix
description: Which VFS types support watch today, and which detection path (push mapping or pull delta hook) each one has.
icon: table-cells
---
...
- **Detection** is per-backend. The **Pull** column marks mounts whose
...
| VFS | Pull (`delta_hook`) | Fingerprint | Push signal (provider-side) | Sample |
| --- | --- | --- | --- | --- |
...
| Box | ✓ `/events` stream (walk is reset) | content `sha1` | Box `/events` long poll, mapped by `BoxEventHook` | — |
...
- **One request per directory**: Google Drive and OneDrive/SharePoint key
  their trees by opaque id and offer no whole-subtree listing, so the walk
  descends folder by folder. SFTP does too, for the same reason. Box has the
  same tree, but its pull reads the `/events` stream instead: an idle pull is
  one request, and the per-folder walk only runs as the reset.
...
class S3Walk:
    async def __call__(self, root):
        async for obj in list_objects(prefix=root.vfs_path):
...
as the reset path. Box `pull` reads the account's `/events` stream from a
stored `stream_position` and places each event through its `path_collection`.
Box never refuses an old position and may repeat an event or send it out of
order, so the tree is walked again once the last walk is two weeks old, and
whenever an event moves the watch root itself or a folder above it. Other
backends with a native cursor (Graph delta, Drive `changes.list`) can do the
same: the opaque checkpoint is the server cursor, and consumers cannot tell the
difference. Keep the walk as the reset path, though. A cursor is a promise the
server keeps, and when it breaks (`path/reset`, `resyncRequired`) a full
listing is the only answer.
